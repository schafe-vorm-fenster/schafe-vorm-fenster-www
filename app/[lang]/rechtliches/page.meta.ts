...
/**
 * TS-WEB-0029 — the `/rechtliches` manifest.
 *
 * [ASSUMPTION, documented per plan/guardrails.md — "ambiguity → documented
 * assumption + open-point entry, then keep working"]: TS-WEB-0029's own open
 * point #5 says a sender surface has no focus job in the four-job sense, and
 * "this spec does not invent the value" for TS-WEB-0006 D1's closed, exactly-one
 ...
 * used here — TS-WEB-0006 D2 itself groups the legal page with the other two
 ...
 * three-job closing block (TS-WEB-0006 D6, same as those two) excludes the same
 * job here. Recorded in `state/open.md`, not decided by rewording TS-WEB-0029's
 * own open point.
 *
 * `liveModules: []` — D7: "fully static, no live data, no client
 * dependencies". Same recorded contradiction with TS-WEB-0006 D1's "≥ 1 live
