...
/**
 * TS-WEB-0024 D1 — the `/dein-kalender` manifest.
 *
 * `request-licence-quote` is deliberately absent — it belongs to
 * `/deine-region` (FUN-WEB-0016); tier 3 links there, it opens no quote here.
...
export const pageMeta: PageMeta = {
  ...
  // Named verbatim as TS-WEB-0024 D1 gives them — three inline proof-bearing
  // slots, not the page's own `data-block` ids (D2's `contrast`/`tiers`/
  // `trust`), which is what TS-WEB-0024-A1 checks against.
