...
provenance: "sourced — 1 generated demo addition under the prototype completeness override (slot 3, reserved testimonial-type placeholder — state/open.md Dummy-Content); EN translation of content/pages/ueber-uns/de.md, same source ids per slot"
...
Price figures from `community-calendar` (free, "forever") and
`portalize-calendar` (480/EUR/year) — read, not typed. Proof:
...
Pool: the full `proof` set plus `media-echo` (32 entries). One place
is reserved for the `testimonial` type — empty today, because all five
testimonial records are `unverified` (Q-014). The empty place is not
backfilled with an element of another type; it shows the hatched area
with a sentence naming the gap (TS-027 D5).

<!-- id: ueber-uns-3-testimonial-slot-demo; content_type: proof-card; provenance: generated; derived_from: []; status: draft; demo: true -->

**Demo testimonial for the reserved place (prototype, `Demo Data` badge):** "The village calendar knows our administration from the inside — you can tell." — Volunteer mayor, Example municipality Musterdorf. Name, place, and quote are entirely invented and recognizably exemplary; the demo card only shows what the reserved place would look like with proof, and replaces no cleared testimonial.
