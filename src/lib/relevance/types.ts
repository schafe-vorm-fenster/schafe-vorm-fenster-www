/**
 * The relevance vocabulary — TS-WEB-0005, the concept's two axes
 ...
 * — so the entry-trait ids are literally one constant (TS-WEB-0010 D3: "two
...
/**
 * TS-WEB-0005 D1, corrected 2026-09-10: five levels, coarsest first. `place` is
...
/** The four jobs of the IA. A page declares exactly one (TS-WEB-0006 D1). */
...
/**
 * TS-WEB-0005 D3. `peripheral` was renamed from `alien`, which read as a verdict
...
/**
 * The entry contexts of the SRC-0002 context matrix, as TS-WEB-0010 D3 names them.
 ...
 * all three (TS-WEB-0005 D2: "no type is without a relation").
 */
...
export interface RelevanceItem<Payload = unknown> {
  /** Stable id — the tie-break of TS-WEB-0005 D6 and the rotation key of D7. */
  readonly id: string;
  readonly type: ItemType;
  /** The **coverage** level, not the venue (TS-WEB-0005 D1). */
  ...
  /** TS-WEB-0005 D4, default 1.0 — multiplies freshness, never replaces it. */
  readonly editorialWeight?: number;
  readonly clearance: Clearance;
  /** Labelled dummy content (plan/guardrails.md, the mock rule). */
  readonly demo?: boolean;
  /** FUN-WEB-0024: the element names a place and may only be shown for a covered one. */
...
/** SRC-0001 §6. Observability and documentation only — nothing branches on it. */
export type PersonalizationStage = 0 | 1 | 2 | 3;

/**
 * The resolver output of TS-WEB-0010 D1: **one flat object**, a field is either
 ...
 * for observability and for the documentation (TS-WEB-0010 D1).
 */
export interface ViewerContext {
  readonly geo: GeoScope;
  readonly trait: EntryTrait;
  /** Fixed per page by the IA. A trait may reorder within it, never redefine it (DEC-0059). */
