...
import { PageSeoMapSchema } from "@/src/domain/content-frontmatter.schema";
...
export function pageSeo(
  ...
  const cacheKey = `${root}:${directory}:${locale}`;

  let map = maps.get(cacheKey);
  if (map === undefined) {
    let raw: string;
    try {
