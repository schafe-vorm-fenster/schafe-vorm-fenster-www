...
(`state/open.md` row 12, Q-0040). The one injection point is
...
parameter, `etcc_*` included — is TS-WEB-0011 D9 and lives in `src/lib/seo`, not
here.
...
- **Field-mapping verification.** D4 rule 4's mapping of `stage` and the
  ...
  `etracker-tracker.ts` until the account confirms it (Q-0040).
