...
| `GET /api/{token}/community/slug/{slug}` | resolving a slug to a community — the app handover (DEC-0029) |
| `GET /api/{token}/community/{id}` | the hierarchy behind the geo tiers (TS-WEB-0005 D1) |

The hierarchy is `place? > community > municipality > county > state >
country`, with `state` required — the five levels TS-WEB-0005 D1 uses.
...
- **Search takes no place name.** Parameters are `countryCode`, `lat`/`lng`,
  ...
  Name search is a demand, not a feature — Q-0025. Until it lands, the
...
- **Proximity search takes no caller radius.** A server-side constant
  ...
  "~15 km" is approximated in the BFF — Q-0038.
- **No coordinate → hierarchy resolution**, which is what county-level
  geolocation would need — Q-0032. And an uncovered place has no
  coordinates at all, so "nearest active place" has no anchor — Q-0051.
- `findbyaddress` exists but is **forbidden** for the website: it triggers
  ...
  visitor's search term to a third party (DEC-0024, TS-WEB-0013 D3).
