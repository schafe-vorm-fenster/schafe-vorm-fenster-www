...
the convention registered as SRC-0011 in
...
three rules hold for all of them (TS-WEB-0008 D2/D10, TS-WEB-0013 D3):

| File | What it is |
| --- | --- |
| `http.ts` | the one way an ecosystem host is reached: a **closed header set** (no client IP, no cookie — TS-WEB-0013-A5), one attempt bounded by `AbortSignal.timeout`, and every non-answer as a single `UpstreamError` type |
| `hosts.ts` | every host and read token, read **only** here — which is what makes TS-WEB-0008-A1 ("no ecosystem host or read token outside `src/clients/*`") a machine-checkable fact |
...
| `geo-api/client.ts` | `community/search` by ZIP and by point, and `community/slug/{slug}`. The address-lookup operation is **not implemented**, deliberately: DEC-0024 forbids it, and a caller cannot reach what does not exist |

Responses are validated with Zod schemas derived from the pinned
`openapi.json` (DEC-0021). The schemas are deliberately **loose on unknown
...
counts as a failure (TS-WEB-0009 D4).
