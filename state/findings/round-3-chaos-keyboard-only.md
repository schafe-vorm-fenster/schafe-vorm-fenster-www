...
| --- | --- | --- | --- |
| C-K-1 | Language switch navigates to the wrong page (`/ueber-uns/archiv` → `/dein-kalender` instead of `/en/about/archive`) | Re-tested href correctness on all 6 routes × 2 locales (12 combinations) and live-activated the link on 6 route/direction pairs (archive de→en, archive en→de, register de→en, register en→de, home de→en, regionQuote en→de). Every `href` matches the exact TS-WEB-0001-A7 equivalent page; every live activation landed on that exact page. | **fixed** |
...
| C-K-7 | Quote form: only the entry point verified | Driven to completion: Tab through Organisation/Name/E-Mail/Telefon/Nachricht, wait past the 2.5 s anti-bot timing gate, Enter on "Absenden"/"Send". `data-envoy-state` flips to `"sent"`, form is replaced by the success message, and focus moves onto it (`role="status"`, `tabindex="-1"`) — exactly as TS-WEB-0016-A9 specifies. Confirmed in both locales at 1280. | **confirmed — full path completes, success focus move verified** |
