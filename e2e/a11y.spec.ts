...
/**
 * The a11y instrument — TS-WEB-0002-A1 and TS-WEB-0029-A12 (F-2-6, round 2).
 *
 * Neither AC had a machine check before this file: no axe dependency
 * existed in the tree (`state/findings/round-2.md` F-2-6), and
 * `e2e/pages/rechtliches.spec.ts` recorded TS-WEB-0029-A12 as a named, skipped
 * test. DEC-0076 is the stack-harmony ADR for `@axe-core/playwright`.
 *
 * Scope of this sweep, deliberately bounded to what the round asked for:
 *   - all 24 (route × locale) pairs of the TS-WEB-0004 D1 inventory
 *     (`everyRoute()`, the same source `e2e/routes.spec.ts` walks)
 *   - two viewports: 360 px (the DEC-0067 phone reference) and 1280 px
 *     (a desktop reference)
 ...
 *     TS-WEB-0002-A1 asks for zero violations, so the sweep now asks for zero.
 *
 * TS-WEB-0002-A1 literally asks for "zero violations … in all three [D4] themes"
 ...
 * turn an experimental rule on, so TS-WEB-0002-A1's "zero violations on every
 ...
 *    each one a distinct name is a copy decision, not a markup fix: TS-WEB-0006
 ...
 *    (TS-WEB-0029, legacy import), so changing them is a content change.
 *
...
// consistent with TS-WEB-0004 D5's per-instance BFF rate limiter (F-2-17) or
...
test("TS-WEB-0002-A1/TS-WEB-0029-A12: the sweep covers all 24 routes", () => {
  expect(ROUTES).toHaveLength(24);
});

for (const { path, locale, route } of ROUTES) {
  for (const viewport of VIEWPORTS) {
    test(`TS-WEB-0002-A1/TS-WEB-0029-A12: ${path} (${route}/${locale}) has no WCAG 2.1 AA violation at ${viewport.name}`, async ({
      page,
    }) => {
      ...
      // F-3-1: **presence**, not impact. TS-WEB-0002-A1 says "zero violations",
