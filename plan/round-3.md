...
3. **The three test files named for C belong to C alone.** A fixes the 404 body (F-2-31); C strengthens the TS-WEB-0004-A4 assertion that currently passes on a placeholder (F-2-55). Same round, different files, no conflict — A must not touch them.
...
- **F-2-32** — `request-product-briefing` dead-ends on all its placements; two pages paste a second placeholder URL against TS-WEB-0016 D7. The real booking URL is in the installed `@schafe-vorm-fenster/people` package.
...
- **F-2-35** — `Q-0020`, `DEC-0027`, `TS-WEB-0007 D12` and friends rendered as visitor copy on 24/24 routes.
...
- **F-2-44** — 11 px and 12 px type below TS-WEB-0002-A10's 15 px floor, plus a bare `28px`. Only the a11y half is in this round.
...
- **F-2-43** — six declared build guards do not exist. Build the three whose inputs exist today (TS-WEB-0002-A3, TS-WEB-0011-A7, TS-WEB-0026-A8); the other three stay blocked on spec decisions.
- **F-2-45** — the landing-only domain rule is not implemented; `kind: "landing"` has no consumer.
- **F-2-55** — `/start` and `/llms.txt` do not exist, and the criteria that guard them assert against the route registry instead of against D1. This is the "tests that assert too little" strand: TS-WEB-0004-A1, A4 and A5.
...
| --- | --- | --- |
| **131** | `/dein-ort`, `/dein-ort/starten`, `/mitmachen/registrieren` and `/dein-kalender/bestellen` are dynamic (ƒ) **by decision** — each reads a request value that *is* the page. Eight of twelve routes prerender. Do not force a shell onto the four; TS-WEB-0020-A10 forbids a skeleton on `/dein-ort` by name | F-2-39, F-2-56 (package B) |
| **132** | A dynamic route cannot hydrate under DEC-0045's hash-only CSP. Choosing a nonce is a DEC-0045 amendment owned **outside** this run, and it is flagged for Jan | F-2-39, F-2-56 (B), F-2-36 (C) |
| **139** | TS-WEB-0023-A9/A10 and TS-WEB-0025-A11 fail against a local production build and pass in dev and on preview — row 132's unhydrated page, not a wiring defect. Do not re-wire the events chasing it | F-2-60, F-2-65 (package A) |
...
| **103, 138** | Titles and meta descriptions still come from the M2 placeholders; closing TS-WEB-0011 D5 needs a schema field plus twelve content files in both languages, and is the content pipeline's, not a fix round's | F-2-43's TS-WEB-0011-A7 guard (C) measures what exists; it does not move the source |
...
| Signal | Decision |
| --- | --- |
| Registration and order flows restart at step 1 on a fresh session; invoice details are lost (C-A-04, C-A-05) | **Not a defect** — TS-WEB-0023-A2/A3 and TS-WEB-0025 D8 fix this as the intended behaviour: the step is in the URL and nothing is stored between page views, which the order page says in its own copy. |
| Form data lost across a language switch (C-A-06) | **Not a defect** — same rule; the route context is preserved, which is what TS-WEB-0001-A7 asks for. |
...
| The nav label "WAS IST LOS" does not say "search for your town here" | **Not a defect** — TS-WEB-0004 D4 fixes the navigation labels; changing them is a concept decision, not a fix round's. |
...
| Order step 1: the search result and "add this place" are two separate clicks | **Not a finding — open-list row.** TS-WEB-0025's scope picker selects several places, so an explicit add is specified; that a first-time visitor expected the search itself to add is a UX-polish row for the M5 budget. |
| `registrieren` commits via `?ort=` where `bestellen` uses `?orte=` plus a chip (C-H-13) | **Not a defect** — two flows, two specs: TS-WEB-0023 registers one place, TS-WEB-0025 selects several. The asymmetry is specified. |
