...
| # | Question | Decision |
| --- | --- | --- |
| 1 | Contact form vs contact section (C1) | **The contact section replaces the contact form.** A new decision supersedes DEC-009 and DEC-010: no general contact form anywhere; the section (video appointment · WhatsApp · phone · mail · portrait) is the one contact surface, and the briefing booking runs through it instead of the external Google link in a hero CTA. Quote and briefing **forms** stay envoy widgets — only the general contact form goes. |
| 2 | One primary CTA per page (C2) | **`TS-006 D3` stands.** Exactly one `data-cta="primary"` per page. Explain-module CTAs and the contact section's first row are secondary. The design guide is corrected, not the rule. |
| 3 | `/ueber-uns` conversion | **Booking becomes the page's primary conversion.** Supersedes `WEB-F-017` and `TS-027 D1`'s `primaryConversion: null`. |
| 4 | The fixed `/ueber-uns` h1 (C4) | **No verbatim copy in specs — as a general rule.** `DEC-036 §3` releases the headline; `TS-027-A3` asserts that an h1 exists and what it must do, never its wording. The same treatment applies everywhere a spec fixes a string or a heading's grammatical form (`TS-024 D6`/`A8`'s question heading included). Copy lives in `content/pages/**`. |
...
| 9 | Price tier order | **Free tier first**, as the IA, `TS-024 D6` and the draft have it. |
...
| 11 | Region pricing and map view | **"Auf Anfrage" without a size qualifier** until a real tier exists in the hub; "nach Größe" is dropped. The map view is **announced with its date** (January 2027, DEC-061), never listed as a shipped feature. |
...
| 13 | Search scope (Q-071) | **Suggestions cover the covered communities only.** A typed name with no match still reaches `/dein-ort/starten` on submit, so the activation path stays intact. Germany-wide suggestions remain the target, carried by the geo-api demand (Q-025). |
| 14 | `Sie` on `/rechtliches` (C9) | **The whole page is exempt** from the register rule, not only the imported documents. |
| 15 | Direct address on `/dein-ort/starten` (C8) | **Direct address everywhere.** `TS-021 D9`'s "never direct" and the `DEC-071 §4` carve-out are dropped; the copy guide needs no exception. |
| 16 | Postcode in the order flow | **Stays.** Drawing a calendar's scope by postcode is purchase configuration, not the visitor's place search; the boundary is recorded in DEC-079 §7 and `DEC-069 §8`. |
