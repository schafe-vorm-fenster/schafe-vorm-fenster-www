...
| CG-025 explain-module step line | `schema` · `e2e` | `max()` per line, **plus** an e2e assertion that neither line wraps at 390 px and that the module with its three lines fits one viewport height (SRC-0014 "Explain module"). Both assertions are **below `lg`**: from the tablet breakpoint the three steps stand side by side and the one-viewport constraint does not apply. |
