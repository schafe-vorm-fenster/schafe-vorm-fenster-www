...
| Q-033 | Which brand package binds? The repo installs `@schafe-vorm-fenster/design-tokens` (generated, from the retired path) while the hub publishes `brand-design` and `brand-identity`. Three candidates, no stated winner. | WEB-C-003 | jan-henrik | TS-017, DEC-043 |
...
| Q-035 | **Nonce or per-build hashes?** A per-request CSP nonce cannot coexist with the static + ISR rendering TS-004 D6 mandates — it turns the prerendered shell into a function invocation, the failure mode DEC-041 §8 forbids. Both variants are written out in TS-014 D3 with their exact `script-src` difference. | WEB-Q-030, TS-004 D6 | jan-henrik | TS-014, 2026-09-10 |
| Q-036 | Where does the `last-good` store live (Runtime Cache · Blob · Edge Config)? An expired `'use cache'` entry is unreadable exactly when upstream fails, so DEC-019 tier 2 needs its own store written on every successful fetch. A cold region falls straight to tier 3. | WEB-F-102, DEC-019 | jan-henrik | TS-009, 2026-09-10 |
...
| Q-039 | Where do `/hilfe/*` legacy URLs redirect? Help lives in the app (WEB-C-010), but the app publishes no help URL contract — two redirect rows have no target, and `content/support/` (37 articles) has no post-relaunch home. | WEB-F-070, WEB-C-010 | jan-henrik | TS-011, TS-018 |
