...
| Q-045 | **BLOCKING — demand to the hub.** No media-echo entry carries `usage_rights` at all (0 of 32). Under "a missing field is no assertion", WEB-F-033's hard clearance filter admits none of them — the archive renders empty and every proof stream that draws on media echo is starved. Add the field, or give TS-005 a source-specific rule. | WEB-F-033, WEB-F-037 | gtm | TS-028, 2026-09-11 |
| Q-046 | **Release blocker — demand to Portalize.** Nothing mints an `organizerId` during an order, so DEC-011's "embed code out immediately" has no mechanism. | WEB-F-015 | portalize | TS-025 |
| Q-047 | **Product gap.** 480 €/year is published for "your places" with no rule for how many, and none for a county — the order flow can assemble a scope it cannot price. | WEB-F-015, WEB-F-020 | jan-henrik | TS-025 |
| Q-048 | Does the enterprise map view actually ship? `portalize-enterprise` promises "calendar plus a map view", its own open points record no confirmed date — the page would sell an unbuilt feature. | WEB-F-016 | offering owner | TS-026 |
| Q-049 | The permanence promise ("bleibt kostenlos", 2022) has **no proof element** — it exists only as a media-echo entry. Mint a proof element, or let TS-005 admit media echo into a reassurance slot. Until then the CTA line is unpublishable (WEB-F-036). | WEB-F-012, WEB-F-036 | gtm | TS-022 |
| Q-050 | "The six current channels" is a figure of speech in `actors.audience.md`, not an enumerated list — so the numeral cannot be written. Enumerate them or drop the count. | WEB-F-012 | gtm | TS-022 |
...
| Q-052 | **Conflict in the sources.** The home brief says the focus job is set by entry context; SRC-001 §6 and WEB-F-052 say a stage may never change the focus job (one exception: the empty place calendar); the conversion map gives `/` only `save-calendar-to-homescreen`. Three statements, one page. | WEB-F-010, WEB-F-052 | jan-henrik | TS-019 |
