...
The PROPOSED review (Q-0058 … Q-0070) put thirteen determinations to a
decision. This record carries the answers that are not large enough for a
record of their own. Q-0058 became DEC-0067, the placeholder principle
became DEC-0068, and Q-0060 and Q-0070 are still open.

## Decisions

### 1. The container stops at 1200 px (Q-0058 residual)
...
greater width and becomes a different layout, which TS-WEB-0017 D2(d) forbids.

### 2. The price is emitted as structured data, with its scope attached (Q-0061)
...
### 3. Eight AAA criteria, not four (Q-0062)
...
TS-WEB-0002 D6 and puts real pressure on dense rows — the archive filter
...
### 4. Stale windows are long; fresh TTLs are not (Q-0067)
...
### 5. The byte budget reports; Lighthouse gates (Q-0069)
...
floor, DEC-0007) — what the visitor experiences is the thing being
promised, and bytes are only a proxy for it.

### 6. Resilience snapshots are committed (Q-0064)
...
### 7. The lead fallback is the Google Form behind `/start` (Q-0066)
...
rules: **linked, never embedded** (DEC-0013 forbids third-party embeds, and
...
`/start` joins the TS-WEB-0004 D1 inventory as the only row that is not a
page: redirect only, `noindex`, absent from the sitemap.

### 8. The order preview is deferred to the backlog (Q-0063)
...
launch. The specification is kept intact in TS-WEB-0025 D4 so the feature does
...
### 9. Context proximity was mislabelled, not undecided (Q-0068)

The endpoints are SRC-0002's: its scoring block states `1.0 starting type
… 0.3 widest widening`. The `[PROPOSED]` tag on TS-WEB-0005 D2 was too broad
...
TS-WEB-0002, TS-WEB-0003, TS-WEB-0004, TS-WEB-0005, TS-WEB-0009, TS-WEB-0011, TS-WEB-0016, TS-WEB-0017 and TS-WEB-0025
are updated. Q-0061 … Q-0069 close in the register. Q-0060 and Q-0070 stay
