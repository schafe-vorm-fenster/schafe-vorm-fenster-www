...
TS-WEB-0004 D3a leaves the mechanism open ("`next-intl` localized `pathnames`, or
an own map consumed by proxy + facade; decided at implementation, the
contract above is what binds"). TS-WEB-0001 D7 adds that a new language must not
...
What TS-WEB-0004 D3a and TS-WEB-0001 D7 actually ask for is a `routeId → { de, en }`
table and a keyed string set; both are typed records:

- `src/lib/routes/routes.ts` — the route translation map, the single source
  for every path, with `href(route, locale)` as the link facade (TS-WEB-0001 D5);
- `src/lib/i18n/locales.ts` — the language table (TS-WEB-0001-A11's "single place
...
the route table; no path is typed in the config. TS-WEB-0004 D3's rules 1 and 2
...
TS-WEB-0011 D1 names `proxy.ts` as its only consumer and rules out config-level
...
not the table. **Status is 301, not Next.js's default 308** — TS-WEB-0001 D4,
TS-WEB-0004-A2 and TS-WEB-0011 D1 all name 301, so the rows carry `statusCode: 301`.
...
- TS-WEB-0004 D3a's contract holds in full: one table, three consumers (URL
...
- Adding a language is a row in `LOCALES`, a path per route, and a
  dictionary object — no call site changes (TS-WEB-0001 D7).
- The 404 body speaks the TLD default on every URL, including an unknown
  ...
  server's view of the path, which DEC-0038 rules out. Recorded as an open
...
- `next.config.ts` carries `experimental.globalNotFound`. It is the only
  experimental flag in the build and it is load-bearing for DEC-0032.
