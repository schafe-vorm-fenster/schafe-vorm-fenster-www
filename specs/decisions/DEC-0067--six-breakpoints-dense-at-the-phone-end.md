...
`lg` 768, `xl` 1024, `2xl` 1280. TS-WEB-0017 D2(b) had fixed **two** — and,
...
DEC-0054 and DEC-0056 had already ruled that the tokens win and the specs
...
a phone (CON-WEB-0002), so the phone is the case to optimise rather than the
...
- TS-WEB-0017 D2(b) lists all six with their purpose; D2(c) takes the
...
- TS-WEB-0017 A4 checks that every `min-width` in the generated CSS is a token
...
- **The verification regime samples the small range.** A criterion that
  ...
  anything. TS-WEB-0017 A8 (layout identity) and A9 (no horizontal scroll)
  sample 360, 428 and 1280.
- TS-WEB-0006 D3 gains 428 × 926 as a third reference viewport, explicitly
...
- The design-system contract's outstanding demand changes shape: a
  ...
  answers.
- CON-WEB-0002 carries the reason, not just the source, so the next reader
