...
- `concept/website-communication-principles.concept.md` (SRC-001) governs
...
- The website specs carry exactly one copy requirement (`WEB-F-008`) and one
  structural determination (`TS-006 D7`), and D7 **requires** the
  question-opener the review rejects.
- `TS-006-A8` depends on a generic-claims term list that its own open point
  records as not existing.
- `TS-007 D5` requires a `describe()` and a `max()` per field with no
...
requirement family, or a longer `TS-006 D7`. That inverts the framework. A
...
(SRC-017), bound to the build by `specs/contracts/copy-contract.md`
(SRC-018).** It is the exact shape the design layer already has: a guide that
says what is right (SRC-014) and a contract that says what the machinery owes
it (SRC-013).
...
3. **A concept document wins over a spec.** `TS-006 D7`'s question-opener
...
- `TS-006 D7` is rewritten: statement openers, reader-directed questions only,
  self-contained blocks, and the generic-claims list now **exists** — it is
  the avoid list of SRC-017 §9 plus the glossary's avoid column. `A8` is the
  static half, `A16` the reviewed half.
- The two page-level criteria that assert a question mark in an opener invert:
  `TS-019-A6` and `TS-022-A4`. Their e2e tests
...
- `TS-007 D5` takes its `describe()` and `max()` values from SRC-017 §6;
  `D9` and `D12` reference SRC-018 for the rows they carry; `D11`'s editorial
...
- `specs/glossary/glossary.md` gains **use** and **avoid** columns per locale.
  ...
  not a licence to invent. Q-057 stays open for the hub import.
- `WEB-F-008` cites the guide and the contract and reaches `S3`.
- The budgets in SRC-017 §6 are provisional against the component manifest
  (SRC-013 §1). The derivation is recorded with them, so a component budget
  replaces a number rather than contradicting one.
- The product-naming consequence stands unchanged: `DEC-052 §1` still
...
- The brand-level rules are being written into the hub in parallel. Until they
  land, SRC-017 §9 carries the word-level anti-patterns the review supplied;
