...
TS-WEB-0003 D2 declared two photographs as LCP elements — a WhatsApp scene on
...
3. **Never a fabricated person, place, or record.** No synthetic
   ...
   DEC-0065 checks text, and an image carries no frontmatter to check.
4. **Never in a proof slot.** Proof elements are governed by
   `usage_rights` clearance (SRC-0002): an uncleared element is excluded,
...
- The two image LCPs in TS-WEB-0003 D2 **stay**. The placeholder exists from
...
- `scripts/make-placeholders.mjs` generates the set from
  ...
  snapshots (DEC-0069): a file that must exist on a clean checkout cannot
