...
2. A place without coverage gets a dedicated flow: "nothing happening
   ...
   this website owns (pending its addition to the IA, per DEC-0022).
3. Name search becomes a new geo-api endpoint backed by the existing
   Typesense index (demand Q-0025). `findbyaddress` is **forbidden** for
...
today, over the committed covered-community index, and `Q-0025` continues
as an upstream demand that blocks nothing that ships (DEC-0079 §2).
...
**Q-0071**; until it is answered, read point 1 as the target, not as the
current state.

**Points 2 and 4 stand unchanged** and still carry FUN-WEB-0047 and the
proof rule.

## Amendment 2026-09-24 — point 1 is settled

The first amendment left point 1 as an open question (`Q-0071`). It is now
answered: **suggestions cover the covered communities, and Germany-wide
finding by name is the target, not the shipped state** (DEC-0079 amendment
2026-09-24).
...
name* is the covered set, until `Q-0025` provides a name endpoint.
