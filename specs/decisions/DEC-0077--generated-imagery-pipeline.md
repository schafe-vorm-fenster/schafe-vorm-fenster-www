...
DEC-0068 filled every image gap with a flat, obviously graphic placeholder:
...
**1 — Generated imagery is allowed as a marked placeholder, under DEC-0068's
...
that make it safe are the ones DEC-0068 already wrote, applied to a
...
- **No fabricated person, no fabricated record.** Every prompt carries, on
  ...
  gesucht" hatch. No generated image enters a proof slot (DEC-0068 rule 4
...
DEC-0068 rule 3's sentence "no invented village" is the one this decision
...
- A background carries no `loading`/`fetchpriority`. The declared image LCP
  that sits on a `photo-surface` (`/mitmachen`, TS-WEB-0003 D2) gets a
...
- A single 21:9 frame centre-cropped into the phone's 8:9 hero box loses its
  ...
  is the base (mobile-first, TS-WEB-0017 D2), the landscape one is swapped in at
...
*runtime* dependencies, TS-WEB-0017 D1) does not gain a row.
...
to WebP down a quality ladder until it clears TS-WEB-0003 D1's 100 KB budget.
