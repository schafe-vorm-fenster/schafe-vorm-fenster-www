...
It also explains an intermittent red: `TS-021-A14` failed once in five full
...
1. **Capture the typed value before hydration.** The window is 96–350 ms;
   ...
   DEC-045's hash-only `script-src` cannot hash a request-time script
   (row 132). Ruled out by the CSP, not by taste.
2. **Move the field into the client.** Costs the no-JS guarantee the
   control exists for (WEB-F-010, TS-019-A11) and does not help: the value
...
4. **Hand `?ort=` on `/` to the place route** (a third row in
   `place-hop.ts`). Would delete TS-019 D2's S2/S3, and with them the
   page's only conversion event (`save-calendar-to-homescreen`, D7) and
   TS-019-A3/A4/A13. A rendering defect does not justify deleting specified
   product behaviour.
...
is a site-wide rule (TS-009 D2, acceptance `TS-009-A14`), enforced against
...
| --- | --- | --- |
| hero content | the headline · lead · CTA trio (`HeroContent`) | TS-019 D2 keys all three on the resolved place |
...
space is identical whichever branch the page renders" — TS-019 D2's own
...
- TS-019 D2's four states, TS-019-A2/A3/A4/A5/A13 and the single
  `data-cta="primary"` per page (TS-006 D3) are unchanged.
...
- `/`'s HTML carries three boundary markers instead of one. The streamed
  ...
  travels twice.
- A future page that puts a form behind a boundary fails `TS-009-A14` in
...
| `e2e/pages/home.spec.ts` (TS-019-A2…A14) | passing | passing |
