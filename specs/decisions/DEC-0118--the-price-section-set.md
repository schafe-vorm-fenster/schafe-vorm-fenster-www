...
8. **Two derived values enter `app/styles/components.css`**: `--height-tag:
   ...
   well of the tier row, the setting row and the banner). Both are composed
   from the design system's stated numbers, like the seven fixed heights
   already there; no colour or family literal is added.
