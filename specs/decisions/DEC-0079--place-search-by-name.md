...
The specification had the premise inverted. `FUN-WEB-0046`, `TS-WEB-0008 D7`,
`Q-0025` and six page specs said: *ZIP works today, name search is blocked
...
2. **Where the names come from is an implementation detail, and it is
   ...
   it may be a geo-api name endpoint (`Q-0025`, geo-api #130 unified
   ...
   store answers her. Swapping the store is not a change to `FUN-WEB-0046`
   or to `TS-WEB-0008 D7`.

3. **The typed name gets a suggestion overlay** (`TS-WEB-0008 D7a`): from the
...
4. **A name with no match is not an error and not a dead end.** It
   ...
   `/dein-ort/starten?ort=<query>` — the founding flow (`FUN-WEB-0047`).
   The search never answers a visitor with silence and never tells her to
   type something else instead.

5. **`findbyaddress` stays forbidden** (`DEC-0024` §3): a paid, slow
...
6. **What this decision does not settle:** whether the search must find
   *uncovered* places Germany-wide. The index knows the covered
   communities only. That is `Q-0071`, and `DEC-0024` §1 is amended to
   point at it rather than to be read as settled.

7. **Out of scope:** the order flow's scope step (`TS-WEB-0025 D3`) keeps its
   ...
   it stays as `DEC-0069` §8 left it.

## Superseded statements

| Where | Said | Now |
| --- | --- | --- |
| `FUN-WEB-0046` | "covers all of Germany; ZIP search works, name search requires the geo-api endpoint" | searches by name over the covered communities; ZIP not offered; source of names free; scope is `Q-0071` |
| `DEC-0024` §3 | name search *becomes* a geo-api endpoint | name search exists on the index; the endpoint is an upstream want (`Q-0025`) |
| `TS-WEB-0008 D7` | ZIP row "works today", name row "blocked on Q-0025", ZIP placeholder as an interim | name is the input; ZIP not offered; the interim sentence is deleted |
| `TS-WEB-0008-A14` | a typed name produces "the documented ZIP hint" | a typed name produces suggestions; a name without a match produces the `starten` destination |
| `Q-0025` | ZIP-only until it lands; blocks `FUN-WEB-0046` | upstream demand; blocks nothing that ships |
| `TS-WEB-0019`, `TS-WEB-0020`, `TS-WEB-0021`, `TS-WEB-0023`, `TS-WEB-0025 D3` | "ZIP-only until Q-0025", "reachable only by ZIP", "bites hardest here" | removed; the criteria that asserted ZIP behaviour assert name behaviour |

## Consequences

- **The specs stop describing a blocker that does not block.** `Q-0025`
  ...
  and would make option 1 of `Q-0071` possible.
...
- **Copy work follows.** Every visitor-facing string that names a
  postcode on a search surface is now a defect against `FUN-WEB-0046`,
  asserted statically by `TS-WEB-0008-A16` — in both locales, in the
  dictionary defaults and in the page content artifacts.
- **The overlay is now specified, not free.** `TS-WEB-0008 D7a` determines
  placement, row count, row format and the no-shift rule; only debounce,
  request cancellation and highlight styling stay free.
- **`DEC-0024` is amended rather than replaced.** Its points 2 and 4 (the
  uncovered place as a conversion moment; proof only from covered places)
  are untouched and still carry `FUN-WEB-0047`.
...
Germany-wide — and named `Q-0071` for it. **Q-0071 is answered**
...
2. **A typed name with no match still reaches `/dein-ort/starten` on
   submit**, with the raw query as `?ort=` (§4, `FUN-WEB-0047`). The
...
3. **Germany-wide finding by name remains the target**, carried by
   `Q-0025` as an upstream demand to geo-api. It is a target, not a
   commitment on a page: no search surface states a limit, states an
   interim, or promises a reach it does not have (§1 unchanged).

`DEC-0024` §1 is re-read accordingly — see its second amendment. §7 (the
