...
TS-WEB-0007 D4 fixes one generated file per **slot** per locale
...
amends TS-WEB-0007 D4's file layout, not its content model.]**
...
baseline of TS-WEB-0013 should never have to accommodate for our own content. The
...
`PageFrontmatterSchema` (the `TS-WEB-####` spec binding of TS-WEB-0017-A14, `route`,
...
defect: a `withheld` or `mixed` slot with no source (TS-WEB-0024 D10, TS-WEB-0026 D5
...
- `loadPage(routeId, locale)` is the whole page-facing surface, and it reads
  ...
  is imported only by the checker and the tests (TS-WEB-0007 D3, A12). There is
...
- Two routes share one artifact: `/deine-region/angebot` is specified with
  `/deine-region` (TS-WEB-0026) and its slots live in that page's file.
