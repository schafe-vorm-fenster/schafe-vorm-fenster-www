...
2. **`order-promotion-material` stays unwired.** No page on the website,
   ...
   a call site is deliberate and recorded, not an oversight. → Q-0005.
3. **Local advertising appears nowhere.** CON-WEB-0015's "at most one
   ...
   nobody can serve. The guard checks for absence. → Q-0006.
4. **Newsletter: footer everywhere, plus inline on `/ueber-uns`.** A
   ...
   one-conversion rule (FUN-WEB-0003) allows precisely because that page is
   conversion-less. → Q-0007.
5. **The Auftragsverarbeitungsvertrag is public**, as a section of
   ...
   procurement hurdle and fits the transparency positioning. → Q-0029.
...
kept. `DEC-0081` §6 gave `/ueber-uns` a primary conversion — the booking
...
`TS-WEB-0027` carried that as an open point addressed to this record's owner.
...
| State | Ask | Rung |
| --- | --- | --- |
| ready to talk | the booking | the page's primary conversion (`DEC-0081` §6) |
| still looking | the newsletter | secondary, below it |

That is exactly the ladder `DEC-0082` introduced: one primary per page,
...
with nothing. Before `DEC-0081` this page asked her for nothing at all,
...
rung — they do not, and `DEC-0082` is what makes the difference
statable.

### 2. Placement and treatment are unchanged

The block keeps the position and weight `TS-WEB-0027 D8` gives it: after the
team block, before the context band, **secondary treatment, never the
primary marker.** `FUN-WEB-0017` / `TS-WEB-0027 D1`'s primary stays
`request-product-briefing`. The one-conversion rule (`FUN-WEB-0003`) is not
being excepted here — under `DEC-0082` it never needed an exception,
...
through a rule that `DEC-0082` has since replaced.
...
`subscribe-to-newsletter` in `@schafe-vorm-fenster/goals` (SRC-0008).

Consequences for the website: the footer entry and this inline block
both carry a named goal, `TS-WEB-0016 D1` S5's empty conversion-goal cell is
filled, and `FUN-WEB-0017` gains the id as the page's secondary.
...
### 5. Q-0020: the system is chosen, the system does not exist

Stated plainly, because §4 and `TS-WEB-0027 D8` both depend on it:

- **The question of *which* system is closed.** `DEC-0051` resolved
  Q-0020: envoy carries signup and double opt-in, as a row of the Q-0022
  contract.
- **No sending system is in operation.** Q-0022's measurement of
  2026-09-11 records it in as many words — "No newsletter operation
  exists either (Q-0020)" — and the envoy widget's delivery date is still
  UNKNOWN (`TS-WEB-0016 D4` C10). Choosing a system did not produce one.
- **Therefore the block ships only when one does.** Neither the footer
  ...
  `TS-WEB-0016 D10` and `TS-WEB-0027 D8` already say this; the amendment makes it
  a property of this decision rather than a rule inherited from a
  tactical spec.

Q-0020 does not reopen. What is owed is delivery, and it is owed under
Q-0022, plus the WhatsApp route of §4, which no system covers today.
