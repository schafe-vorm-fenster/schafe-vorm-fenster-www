...
icon package in one sitting. TS-WEB-0017 D1 forbids a second rendering framework and
...
e2e, ESLint 9 with `eslint-config-next`, TypeScript 5.9. DEC-0002's rule that a
...
2. **No Tailwind, no component library — plain CSS and CSS Modules.** TS-WEB-0017
   ...
   CSS wins on the two rules that are *not* free: TS-WEB-0017-A4 asks that every
   `min-width` in the generated CSS be a `breakpoint.*` token value, and
   TS-WEB-0017-A5 that no colour or `font-family` literal exist outside one file.
...
**Lucide is the icon set** (TS-WEB-0017 D7, DEC-0056), as `lucide-react`.
...
tool that needs it. They go on `state/open.md` because TS-WEB-0014 D12 will gate
...
1. **The Vercel project still ran `yarn install` and `yarn build`**, left over
   ...
   dashboard setting regardless. The file carries **no `headers` block**: TS-WEB-0014
...
- `pnpm check` is the single gate and grew two checks: `check:stack`
  (TS-WEB-0017-A1, A2, A7, A17) and `check:brand` (TS-WEB-0017-A4, A5, A6). Both are
...
- The deny-set in `stack.allow.json` is [PROPOSED] — no source enumerates
  forbidden frameworks (TS-WEB-0017 D1 says so itself). It is changed by a
  decision, not by a commit.
- `knip` and `jscpd` are *not* adopted yet. They belong to the `Quality` job
  of TS-WEB-0015 D4, which is stage-2 pipeline work; adopting the tools before the
