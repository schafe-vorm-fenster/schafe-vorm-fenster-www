...
1. **Geo hierarchy follows the geo-api**: `country > state > county >
   ...
   (TS-WEB-0005 D1). GTM's model packages follow later; the website does not
   wait for them.
...
7. **Segmented, not personalised.** Dynamic elements are segmented by
   ...
   cache cost of that resolution is measured before launch (Q-0030).
...
→ TS-WEB-0005. Editorial weight is an input, not a formula term. Weights are a
profile per focus job, which settles Q-0004 (job fit may outrank geo, and
where is a property of the page). Two risks are now tracked: cache cost
of community segmentation (Q-0030) and assessment drift of the sticky
facets (Q-0031).
