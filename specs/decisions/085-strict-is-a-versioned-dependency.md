...
explicitly". `specs/README.md` repeated it, `WEB-C-006` carried the path as
...
read. The same restructuring that produced `DEC-042` for hub content applies
...
`DEC-042` settled this for hub content; it now covers the method too. A spec
that needs a STRICT rule cites
`@leafcutter-strict/method-statement-grammar`, not a directory on a laptop.
`AGENTS.md`, `specs/README.md`, `WEB-C-006`, `TS-017 D6` and `GL-020` are
rewritten accordingly.

Two places keep the old path on purpose: `DEC-023`, which is the record of
...
What did change is `TS-017-A2`: the `.npmrc` assertion now covers the
...
weight profile, the tone profile of `DEC-066`, the `person-profile`
...
| --- | --- | --- |
| Identifier schema | `<TYPE>-<DOMAIN>-<NNNN>` — `FUN-WEB-0001` | `WEB-F-###`, `TS-###`, `DEC-###`, `Q-###`, `SRC-###` are cited from sibling repositories and from every document in `plan/reviews/`. `method-identifier-and-locator-schema` forbids renumbering in the same breath; the id we hold is the artefact's. Cost: a rename of ~340 ids in ~155 files plus every external citation |
| The business-rule class | FUN · NFR · CON · BUS | This repository has three classes. A fourth needs its own id family and an extraction pass over 155 rows |
| Statement grammar | per-class slot forms, one modal one predicate, `form` on the artefact | 155 shall-form prose statements would be re-cast. Mechanical for none of them |
...
| Decision policy | `DP-01…DP-14`, four executor modes, `POL-*`, `SDR-####-####` | No blueprint selects a policy on purpose — which of conservative, assisted or delegated binds here is an owner decision, not an install default |
...
| Acceptance criteria shape | `acceptance_tests` with given/when/then | `DEC-040` chose id-carrying criteria with a verification level over a uniform format, deliberately. 420 of them exist |
| Per-artefact versions | `method-version-increment` | No artefact here carries a version field |
...
- A STRICT upgrade is a version bump with a diff, and `pnpm check:specs`
  ...
  That is the point.
- `WEB-C-006` names the package and its version instead of a laptop path.
