...
`DEC-036` §3 fixed the `/ueber-uns` headline word for word, and
`TS-027-A3` asserted that exact string in an e2e criterion. The
2026-09-22 review rejected the sentence as untrue — the site runs in a
data centre — and SRC-017 CG-033 now puts it on a build-failing avoid
...
wording. `TS-024 D6` and `TS-024-A8` require the tiers block to carry
...
`TS-022 D7` fixed a CTA label; `TS-006 D5` phrased a context-band entry as
a rhetorical question CG-006 forbids; `GL-004` still defined a scene as an
"aha question"; `TS-021 D9` quoted a sentence from SRC-002 verbatim as a
rule about tone.
...
2. **No fixed grammatical form.** "A question heading", "a statement
   ...
   live in SRC-017. A spec names the job the heading does: it frames what
...
3. **Copy lives in `content/pages/**`** and is governed by the copy guide
   (SRC-017) and its contract (SRC-018). A rule about how the site speaks
   is a concept, and a concept is where it is written — the same division
   `DEC-080` drew for the guide itself.
4. **Four things are not copy**, and stay where they are:
   - **A forbidden literal inside a check.** A lint needs the string it
     fails on (`TS-024-A10`'s `4.000`, `TS-026-A2`'s `Umkreis`,
     `TS-008-A16`'s `Postleitzahl`). Naming what may not appear is not
   ...
   - **A label owned by a registry or by law.** `Impressum`,
     ...
     registry (`WEB-F-029`, `TS-004 D8`).
   - **A system label fixed by a decision.** The freshness label of
     `DEC-019` and the placeholder badges of `DEC-077` are markers the
   ...
   - **A data format.** "place (municipality)" as the shape of a
     suggestion row (`DEC-079` §3) describes which fields appear in which
     order, not a sentence.
5. **`DEC-036` §3 is amended**: the `/ueber-uns` headline is released. What
...
- `TS-027 D3`/`A3` assert an `h1` and what it must achieve; the sentence
  is the content phase's, under CG-020 and CG-033. `TS-027`'s open point
  about the headline's punctuation is void — there is no headline in the
  spec to punctuate.
- `TS-024 D6`/`D6a`/`A8`: the tiers block needs **a heading that frames
...
- `TS-022 D7`'s CTA label, `TS-006 D5`'s rhetorical example,
  `TS-021 D9`'s quoted sentence, `TS-026 D4`'s counter sentence,
  `TS-023`'s step-indicator example, `TS-009 D9`'s failure sentence and
  `GL-019`'s empty-state sentence become descriptions of the job the
  element does, each citing the CG rule that governs its wording.
- `GL-004` defines a scene by its structure — a concrete, picturable
