...
TS-WEB-0009 D4 fixes the three-tier fallback chain and is explicit that the render
...
TS-WEB-0009's own open points leave the store undecided and name three candidates
...
jan-henrik. DEC-0045/046 already point at the Runtime Cache. The run may not
...
segment key, TTL = the serve-stale window of TS-WEB-0003 D5 for that data kind,
...
- **No store at all** (in-process memory only) — on serverless the instance
  ...
  on tier 3. That empties TS-WEB-0009 D4 of its middle tier.
...
and the alternative costs a durable store. The question TS-WEB-0009 asks
...
no page. `src/lib/live/cache-profiles.ts` carries TS-WEB-0003 D5's numbers in both
...
- Tier 2 is only as good as the region's cache. Measuring its hit rate
  belongs with Q-0030 (cache cost of community-level segmentation), which
