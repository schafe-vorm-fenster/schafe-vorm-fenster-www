...
ask the visitor to classify herself, which FUN-WEB-0009 forbids.

## Consequences

- **Q-0047 dissolves.** The order flow cannot assemble an unpriceable
  scope, because scope does not drive price. Scope determines what the
  calendar *shows*.
- TS-WEB-0024 D6's tier module is restated around place of use.
- TS-WEB-0025's scope selection is a configuration step, not a pricing step.
- `/deine-region` keeps its name (DEC-0036 §4's reasoning is corrected:
