...
| Decision policy | `DP-01…DP-14`, four executor modes, `POL-*`, `SDR-####-####` | No blueprint selects a policy on purpose — which of conservative, assisted or delegated binds here is an owner decision, not an install default. **It now has a consequence** (DEC-0087 §4): with no policy there is no decision point, so `check:specs` E15 refuses any status but `DRAFT`, and W5 reports what the evidence would support |
