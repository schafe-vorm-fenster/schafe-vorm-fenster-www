...
| WEB-C-011 | Prices are published only for offerings with `promotion: promoted`; enterprise is on request; `portalize-website-widget` and `local-advertising` (`withheld`) are not offered on the website. | SRC-001#boundaries, `go-to-market-os/offerings/` | S2 |
...
| WEB-C-014 | The product name "Portalize" is never a navigation label; whether and where the name is introduced on the page (e.g. at the 480 € tier, before it appears on invoices) is open (Q-012). | SRC-003#navigation | S2 |
| WEB-C-015 | Companies/local advertising: while `withheld`, at most one sentence on `/eigener-kalender` (Q-006 tracks the final form). | SRC-003#open-points | S1 |
