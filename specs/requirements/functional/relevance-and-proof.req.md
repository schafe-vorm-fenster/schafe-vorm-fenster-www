...
| WEB-F-038 | Geo proximity shall be computed against the five-level location model `place · municipality · county · state · country` carried by `geo:` frontmatter in `@schafe-vorm-fenster/media-echo` (complete, 34/34) and `proof/`. | SRC-002#required-data | S2 |
