...
status: DRAFT
sources: [SRC-003]
decisions: [DEC-009, DEC-010, DEC-011, DEC-012, DEC-051, DEC-052, DEC-081]
---
...
| WEB-F-093 | A booking action on any page shall resolve to that page's contact section; the section's first action row shall be the Google Calendar appointment link (outbound navigation, no embed). `request-product-briefing` completes on that row's activation, not on the in-page action. **Every row of the section additionally carries `make-contact`**, counted per channel and with the route, and named as an **intent** — three of the four rows hand the visitor to another application, so the contact itself is not observable. An in-page action emits nothing. | DEC-010, DEC-081 | S3 |
...
| WEB-F-096 | Newsletter signup shall carry `subscribe-to-newsletter` and shall offer **both** its channels — e-mail and WhatsApp, WhatsApp preferred (DEC-052 §4 as amended); a surface offering e-mail only does not satisfy the goal. The e-mail route implements double opt-in through envoy (DEC-051), as a row of the Q-022 contract; the WhatsApp route is a click-to-chat link with a prefilled subscribe message. Both routes are cookieless and GDPR-compliant, and neither ships before a sending system is in operation. | DEC-051, DEC-052 | S3 |
