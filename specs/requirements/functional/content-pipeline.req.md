...
| WEB-F-086 | Feed-like content — `media-echo/verified/` for `/warum-wir/archiv` and inline proof — shall be fetched at build time rather than versioned as a package. There is no news section (DEC-022; the IA is authoritative). | SRC-009 ADR-001, DEC-022 | S3 |
| WEB-F-087 | Page copy production happens after this specification phase (project rule); specs use placeholders. | DEC-023 | S3 |
| WEB-F-088 | Legal texts (imprint, privacy, terms) continue to arrive via the Google Workspace import pipeline (`content/legal/` + `import.yaml`); they are the one content type not sourced from the GTM hub. | DEC-012 | S3 |
