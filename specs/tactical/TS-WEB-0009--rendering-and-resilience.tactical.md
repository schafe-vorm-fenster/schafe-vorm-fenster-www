...
*Which* route renders in which mode is TS-WEB-0004 D6; *how long* anything is
cached is TS-WEB-0003 D5. This spec fixes the mechanism both of them assume.

## Determinations

### D1 — Three rendering layers [FIXED: DEC-0019, DEC-0041 §8; realisation PROPOSED]
...
| **Cached island** | proof stream, dates per place, nearby dates, active places, region examples, counters | `'use cache'` component with `cacheLife` + `cacheTag` per TS-WEB-0003 D5, keyed by its props | only on a cache miss, and then behind a `<Suspense>` boundary |
...
Consequence for TTFB (< 200 ms, TS-WEB-0003 D1): TTFB measures the shell only.
...
per-visitor value into a per-segment cache entry (TS-WEB-0005 D8).
...
| entry trait | derived from route + referrer/campaign params | dynamic resolver | proof stream (TS-WEB-0005 D8) |
| `Save-Data` | `headers()` | dynamic resolver | image variant choice (TS-WEB-0003 D6) |
| ISO week seed | computed, not read from a request | resolver, passed as prop | proof stream (TS-WEB-0005 D7) |
...
branch (DEC-0078). A boundary is revealed by *replacing* DOM: React inserts
...
### D3 — Module inventory and its layer assignment [FIXED: TS-WEB-0004 D6, TS-WEB-0003 D5; assignment PROPOSED]

Consistent with TS-WEB-0004 D6 ("all content pages: static + ISR, live modules
streamed via Suspense with skeletons"). Cache lifetimes are TS-WEB-0003 D5's;
they are repeated here only as a pointer, never as a second source.

| Module | Layer | Props (cache key) | `cacheLife` (TS-WEB-0003 D5) | `cacheTag` | Fallback |
| --- | --- | --- | --- | --- | --- |
...
(TS-WEB-0004 D6) — fully static, therefore trivially outage-proof.

### D4 — The three-tier fallback chain [FIXED: DEC-0019; realisation PROPOSED]
...
| --- | --- | --- | --- | --- |
| 1 live | upstream answers within the timeout, response validates | upstream (via `src/clients/*`) | writes `last-good:{key}` in the Vercel Runtime Cache (DEC-0046) with `fetchedAt` | none, unless older than its fresh TTL |
| 2 stale | upstream errors, times out, or fails schema validation | `last-good:{key}` inside its serve-stale window (TS-WEB-0003 D5) | — | freshness label, D5 |
...
- **Failure is anything that is not a valid answer**: network error,
  ...
  the pinned OpenAPI spec (DEC-0021). A malformed 200 must not reach a
  page.
...
### D5 — Freshness envelope and the "Stand: …" label [FIXED: DEC-0019, TS-WEB-0003 D5; format PROPOSED]
...
localized per TS-WEB-0001, wrapped in `<time datetime="…">` with the ISO
...
| tier 2, `last-good` within its serve-stale window (24 h, TS-WEB-0003 D5) | figures **plus** the timestamp of D5 — mandatory here, not conditional |
...
To keep the collapse out of CLS (< 0.1, TS-WEB-0003 D1 / A7), the counter band
is **never placed above the fold** and is never an LCP element (TS-WEB-0003
...
| active places / map | fixed aspect-ratio box (map) or 5 rows (list, TS-WEB-0003 D6) |
| region examples | 3 cards, fixed card height |
| proof stream | element count per surface (Q-0003), fixed card height |
| live counters | one band, three figure slots |

### D8 — Build-time snapshot artefact [FIXED: DEC-0069]
...
| Validated | against the same Zod schemas as runtime responses (DEC-0021); an invalid answer does not become a snapshot |
...
| Refresh | every deployment (TS-WEB-0003 D5, "until next deploy") |
...
### D9 — Failure containment [PROPOSED, within DEC-0032]
...
- Every island sits in **its own error boundary**. A throwing island
  ...
  broken page (DEC-0032: module errors stay invisible).
- The route-level `error.tsx` catches only genuine render errors of the
  shell; `global-error.tsx` stays fully static (TS-WEB-0004 D6).
...
- The place search is a client interaction, not an island: it keeps its
  ...
  copy (SRC-0017 CG-032; DEC-0083).

### D10 — Guard against the per-request-function failure mode [FIXED: DEC-0041 §8; enforcement PROPOSED]
...
| CI asserts from the build manifest that every route of TS-WEB-0004 D1 emits a **prerendered shell**, and that the number of fully dynamic routes is zero | CI |
| Production watch: function invocations per route are monitored; a route whose invocation count approaches its request count has regressed | RUM (DEC-0017) |
...
- [FREE] Visual design of the skeletons — colours, radii, shimmer
  treatment — within D7's geometry rules and TS-WEB-0002.
...
| ID | Level | Check |
| --- | --- | --- |
| TS-WEB-0009-A1 | static | `cacheComponents: true` is set; no `cookies()`, `headers()` or `searchParams` access occurs inside a `'use cache'` module (build succeeds, lint rule green). |
| TS-WEB-0009-A2 | tool | Build manifest: every TS-WEB-0004 D1 route emits a prerendered shell; zero routes are fully dynamic. |
| TS-WEB-0009-A3 | integration | With every upstream stubbed to a 5 s delay, the shell of each content page responds within the TTFB budget and contains header, footer, copy and all module skeletons. |
| TS-WEB-0009-A4 | unit | `resilient()`: valid answer → tier 1 and a `last-good` write; upstream error/timeout/schema failure → tier 2 from `last-good`; empty or expired `last-good` → tier 3 snapshot; an empty valid answer stays tier 1. |
| TS-WEB-0009-A5 | integration | Each D3 island is cached under the props listed there with the `cacheLife`/`cacheTag` of TS-WEB-0003 D5; two visitors of one segment produce one cache entry. |
| TS-WEB-0009-A6 | e2e | Simulated full API outage, warm cache: every page renders tier-2 content with a "Stand: …" label; status stays 200; no error text appears in any module. |
| TS-WEB-0009-A7 | e2e | Simulated outage, cold cache: every module renders its tier-3 snapshot with a label; the counter band is absent from the DOM. |
| TS-WEB-0009-A8 | tool | CLS < 0.1 on every content page in both counter branches — counters present and counters hidden — with all islands streaming. |
| TS-WEB-0009-A9 | integration | For every D3 island, the skeleton's rendered box equals the resolved module's box (height within 2 px); no skeleton renders a spinner or a placeholder figure. |
| TS-WEB-0009-A10 | unit | The freshness label appears exactly under D5's four conditions, carries a valid `<time datetime>`, and is localized. |
| TS-WEB-0009-A11 | integration | An island whose fetcher throws does not affect the route: page status 200, the other islands render, the failing island shows its fallback state. |
| TS-WEB-0009-A12 | tool | Snapshot build step produces a file for every D3 module with fallback "tiers 1–3", produces none for counters, and fails the build if a required file is missing. |
| TS-WEB-0009-A13 | manual | Screen reader: skeletons are not announced; the `/dein-ort` empty state is announced once on arrival. Under `prefers-reduced-motion` no skeleton animates. |
| TS-WEB-0009-A14 | e2e | On every TS-WEB-0004 D1 route, the shipped HTML contains no `<input>`, `<textarea>` or `<select>` inside a streamed boundary — neither in a pending segment nor in the resolved branch parked at the end of the body (D2's last rule, DEC-0078). |
...
- **"Stand: …" in the other languages** (D5) needs the localization pass
  of DEC-0026; the EN wording proposed here is not confirmed copy.
