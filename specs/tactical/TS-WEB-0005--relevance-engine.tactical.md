...
### D1 — Geo hierarchy [FIXED: DEC-0041, corrected 2026-09-10]
...
### D2 — Context proximity [FIXED: SRC-0002 scoring block; the middle value and the floor rule PROPOSED]

Every element type has a defined proximity to every entry context — no
type is without a relation. The entry context (SRC-0002 context matrix)
...
| --- | --- | --- |
| starting type | 1.0 | SRC-0002: *"1.0 starting type"* |
| named widening type | 0.6 | this spec — see below |
| any other type | 0.3 | SRC-0002: *"… 0.3 widest widening"*, extended to types the matrix does not name |

The endpoints are the source's, not this spec's: SRC-0002's scoring block
states `context_proximity(e, p)  # 1.0 starting type … 0.3 widest
widening`. Two things the source leaves open, and this spec closes:

- **The middle.** SRC-0002 describes a span, not a gradient. Every named
...
SRC-0002 and lives with the engine as data, not code.

### D3 — Job relation [FIXED: DEC-0041, corrected 2026-09-10]
...
### D4 — Time and editorial weight [FIXED: DEC-0041; steps PROPOSED]
...
to `w_time` and `w_job` — **0.20 and 0.15** (DEC-0048), giving `w_time 0.35 · w_ctx 0.25 · w_job 0.40`.
...
**Place-bound elements come only from covered places** (FUN-WEB-0024):
...
(FUN-WEB-0031). Kept deliberately simple, to be optimised later:
...
4. Stop at the element count for the surface (DEC-0048): 3 inline beside a claim · 5 on the home page · 7 in the `/ueber-uns` stream.

### D7 — Determinism and rotation [FIXED: DEC-0041]
...
### D8 — Segmentation and caching [FIXED: DEC-0041]

The engine never renders per visitor. Two segmentation axes:

| Axis | Resolution |
| --- | --- |
| geo | **community** — falls back to municipality if production cache figures demand it (DEC-0055, a parameter change) — corrected 2026-09-10: municipality made tier 0 unreachable, since the engine would never learn the visitor's community and could not distinguish tier 0 from tier 1. Community makes the place effect real, at roughly 10–20× the cache entries. That trade is accepted. |
| entry trait | the entry contexts of SRC-0002, from which the focus job derives |
...
per segment rather than per visitor. Each carries `cacheLife` per TS-WEB-0003
...
### D9 — Placement in the codebase [FIXED: DEC-0041]
...
| ID | Level | Check |
| --- | --- | --- |
| TS-WEB-0005-A1 | unit | Geo tiers 0–6 per D1 for every level combination, including `state`, foreign-country elements, and elements without geo. |
| TS-WEB-0005-A2 | unit | Clearance filter removes uncleared elements before scoring; a high-scoring uncleared element never appears. |
| TS-WEB-0005-A3 | unit | Scoring reproduces SRC-0002's worked example — visitor from Lehre, stage 1: positions 1–7 in the documented order. |
| TS-WEB-0005-A4 | unit | Determinism: identical input yields identical output across 1000 runs; ties resolve by id. |
| TS-WEB-0005-A5 | unit | Stage 0 (no geo): `w_geo` is 0 and the order is driven by time and job fit. |
| TS-WEB-0005-A6 | unit | Editorial weight: an element five years old with weight 2.0 outranks a fresh element of equal other scores. |
| TS-WEB-0005-A7 | unit | Ordering never deadlocks: with candidates on only one side, the stream still fills. |
| TS-WEB-0005-A8 | integration | The proof stream component is cached per `{municipality, trait, job}`; two visitors from the same municipality with the same trait hit one cache entry. |
| TS-WEB-0005-A9 | integration | The route shell renders prerendered without waiting for the engine; the static default appears before the segment variant. |
| TS-WEB-0005-A10 | e2e | Place-bound proof shown to a visitor always references a covered place. |
| TS-WEB-0005-A11 | unit | Rotation: two different ISO-week seeds reorder only equally-scored candidates; the ranking by score is identical. The engine reads no clock — the seed is an input. |
| TS-WEB-0005-A12 | unit | Ordering: given more than three elements the engine produces the D6 sequence and never a chronological one; ties break by id. **Limitation:** this proves the engine orders correctly, not that every list on the site uses it — a spot check on two rendered pages (A13) is the only coverage of that. |
| TS-WEB-0005-A13 | e2e | Spot check on `/` and `/ueber-uns`: the rendered proof stream is not in date order. The archive is the documented exception (TS-WEB-0028 D2) and is excluded. |
| TS-WEB-0005-A14 | unit | For each entry context of the SRC-0002 matrix the engine selects the documented starting type and time window; a type named in no row scores the widest step. |
| TS-WEB-0005-A15 | static | Every claim declared in content frontmatter resolves to a proof id that exists and is cleared, or is explicitly marked as having no proof — in which case the rendered claim carries its weakened form. A declared claim with a dangling proof id fails the build. |
| TS-WEB-0005-A16 | unit | The widening chain resolves in the documented order (place → surroundings → county → all regions) and each step sends the id list of its level; no step is skipped when the previous one returns results. |
...
- Q-0002 (stage-0 split), Q-0003 (element count per surface), Q-0004 (may
  job fit outrank geo).
