...
Boundaries: *which* elements are selected and ordered is TS-WEB-0005 — this
spec only guarantees the facets TS-WEB-0005 reads. *Where* content is rendered
is TS-WEB-0004. Layer C (page compositions) has no tactical spec yet (open
points).
...
This is what discharges FUN-WEB-0039: media-echo entries carry no
`audiences[]`, and they no longer need to — DEC-0041 §2 and TS-WEB-0005 D3 make
job relation an **assessment** rather than a derivation from audience
tags, so the missing field weakens nothing and blocks nothing. Q-0019
...
| request time | `content/` local files, app APIs via TS-WEB-0004 D5 | any hub package, any GTM artefact, `npm.pkg.github.com` |
...
(TS-WEB-0004 D6). There is no news section (DEC-0022). [PROPOSED — amends the
...
### D5 — The Zod schema hierarchy [FIXED: FUN-WEB-0089, DEC-0020, concept B.1–B.3; field lists PROPOSED]
...
| Rule | Realisation |
| --- | --- |
| authoring guidance is machine-readable | every field carries `describe()`: what belongs in it, phrasing, tone — naming the `CG-###` rule of SRC-0017 that governs it, rather than paraphrasing it |
| length budget is machine-readable | every field with a visual limit carries `max()`; the number comes from SRC-0017 §6 until the component manifest (SRC-0013 §1) supersedes it, and §6 records the derivation so the replacement is comparable |
...
| `ContentLocaleSchema` (`de`/`en` enum) | becomes a locale **set** derived from configuration, never a hard-coded pair (DEC-0006, D8) |
...
### D6 — Provenance is the update key [FIXED: FUN-WEB-0083, DEC-0020 §5, concept B.1]
...
### D7 — The relevance facets the pipeline owes TS-WEB-0005 [FIXED: TS-WEB-0005, DEC-0041, concept B.5]

TS-WEB-0005 reads this schema and nothing else. Selectable types therefore
...
| `place_bound` | plus the place reference, so TS-WEB-0005 D5 can ask events-api before showing "in <place>" (FUN-WEB-0024). |
...
3. **Original artifacts stay original** (DEC-0026): a clipping headline, a
...
*avoid*, per locale (DEC-0080). They are filled only where the review of
...
SRC-0017 §9, and SRC-0018 binds both lists as the single avoid list
`check:content` reads (CG-040). Q-0057 stays open for the hub import; it
...
`specs/contracts/copy-contract.md` (SRC-0018), which assigns every rule of
SRC-0017 to a mechanism — schema `max()`/`describe()` (D5), one of these
...
**Trigger mechanics are UNKNOWN (Q-0018).** Options on the table: a
...
| ID | Level | Check |
| --- | --- | --- |
| TS-WEB-0007-A1 | unit | Every content type parses a valid fixture; an unknown `type` fails; a field over its `max()` fails; a type declaring a source entity it may not derive from fails. |
| TS-WEB-0007-A2 | tool | `check:content` resolves every `derived_from` entry against the installed packages; an unresolvable ref, a malformed ref, and an empty list each fail with the file named. |
| TS-WEB-0007-A3 | tool | Clearance re-validation: a fixture whose installed package revoked `usage_rights` makes the run exit non-zero and name file plus record. |
| TS-WEB-0007-A4 | integration | The same revocation **fails the build** — no deploy artefact is produced. It is not logged as a warning. |
| TS-WEB-0007-A5 | tool | Locale completeness and slot binding: a missing `en` sibling fails; an orphan content file fails; a required slot with no file fails. |
| TS-WEB-0007-A6 | unit | Selectable types: a missing facet fails; an unassessed job parses as the lowest step and is marked unassessed; a non-default `editorial_weight` without its reason fails; an absent geo level is `null`, never inferred. |
| TS-WEB-0007-A7 | unit | The source adapter resolves `<package>@<version>#<record-id>`, and fails loudly on a version mismatch with the installed package and on an unknown record id. |
| TS-WEB-0007-A8 | static | No content file and no source reference contains a repository path; every `@schafe-vorm-fenster/*` content package is a devDependency and is imported from no request-time module. |
| TS-WEB-0007-A9 | unit | Harmonisation: locale variants of one id agree on records, claims, proof bindings, CTA target and conversion goal, filled slots, numbers and `UNKNOWN` markers — divergence there fails; differing phrasing, sentence count and length within budget pass. |
| TS-WEB-0007-A10 | tool | Every audience, conversion goal, offering and proof id used resolves in an installed package; no such id is defined locally. |
| TS-WEB-0007-A11 | integration | `content/legal/<locale>/` renders as the anchored sections of the one legal page in registry order; anchors match TS-WEB-0004 D8; a legal file carrying generation-only fields fails validation. |
| TS-WEB-0007-A12 | integration | No request-time code path reads a hub package or contacts `npm.pkg.github.com`; runtime content reads resolve inside `content/` only. |
| TS-WEB-0007-A13 | tool | Glossary conformance: a banned term in a field where it is banned is reported with file, field and term. |
| TS-WEB-0007-A14 | integration | A production build contains only `status: approved` content; a `draft` file renders in preview and reaches no production page. |
| TS-WEB-0007-A15 | manual | P7 dry run on one bumped package version: the PR touches exactly the files whose `derived_from` names a changed record, resets them to `draft`, and carries `job_relation` and `editorial_weight` plus their reasons forward unchanged. |
| TS-WEB-0007-A16 | tool | Segment independence: no generated string contains a resolved place name; place references occur only as named interpolation slots. |
...
- **Clearance staleness has no trigger.** D12 check 4 and D13's immediate
  ...
  channel as Q-0018, or a separate, faster one? Until answered, the
  guarantee is "fails at the next build", not "removed on revocation".
- **Assessment drift (Q-0031).** `job_relation` and `editorial_weight` are
...
- **Q-0018, update trigger mechanics.** D13's steps hold under any
...
- **Length budgets are provisional.** D5 requires `max()` from the
  component; the components do not exist. Interim: **SRC-0017 §6**, which
  derives every budget from the SRC-0014 type scale at 390 px and records
...
- **Glossary columns.** ~~The `GL-####` register carries meanings, not copy
  words.~~ **The columns exist** (DEC-0080): *use* and *avoid*, filled from
  ...
  stays in `specs/` once `@schafe-vorm-fenster/glossary` ships (Q-0057,
  DEC-0062).
- **Legal import owns no locale dimension yet.** `import.yaml` lists five
  documents with single targets while DEC-0027 requires DE and EN.
