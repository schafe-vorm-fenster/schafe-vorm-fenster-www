...
| **The one-viewport rule and the advance's trigger do not meet at every phone height.** `DEC-0105 §6` starts the advance when the **whole module** is in the viewport; the one-viewport rule is authored against 360 × 800 (`SRC-0014`'s "per screen") while `TS-WEB-0006 D3`'s fold viewport is 360 × 640, so on a short phone a module that honours its budget still never fits entirely on screen and the trigger, read literally, never fires. `DEC-0105 §6` proposes the graphic stage in full plus the first step line as the fallback and flags it `[PROPOSED]`. **Answered by:** the owner of the motion exception, as **Q-0083**. | jan-henrik |
