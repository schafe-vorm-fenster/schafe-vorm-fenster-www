...
TS-WEB-0002 and TS-WEB-0003; this spec fixes *when they run and what they block*.

## Determinations

### D1 — Two stages, one pipeline grown in two steps [FIXED: DEC-0031]
...
### D2 — Branches, environments, hosts [FIXED: DEC-0031, DEC-0035; branch names PROPOSED]
...
One Vercel project (DEC-0002), not two. The migration preview is a
...
after the product's pre-launch preview vacates the host (DEC-0035) — an
...
### D3 — Protection and `noindex` on everything that is not production [FIXED: CON-WEB-0020, CON-WEB-0023]
...
indexable  =  process.env.VERCEL_ENV === "production"
              && requestHost ∈ canonicalPublicHosts   (TS-WEB-0004 D1 domain set)
```
...
### D4 — Merge-gate job graph, feature branches [FIXED: SRC-0012, CON-WEB-0021; names and layout PROPOSED]
...
| 4 | `Budgets` | 3 | Lighthouse CI on the five TS-WEB-0003 D7 routes · bundle guard (TS-WEB-0003 D7) · axe-core sweep (TS-WEB-0002 A1) — all against `deployment_url` with the bypass header | yes |
...
Deviation from SRC-0012, deliberate: **no `paths-ignore: "**/*.md"`.**
...
### D5 — What blocks a merge [FIXED: CON-WEB-0021; the required-check list PROPOSED]
...
| Lighthouse Performance < 98 · bundle over budget | yes | TS-WEB-0003 D7 owns the numbers |
| axe-core violation | yes | TS-WEB-0002 owns the rule set |
...
### D6 — Auto-merge rules [FIXED: SRC-0012]

Two workflows, adopted from SRC-0012 unchanged in intent.
...
### D7 — Production promotion: rolling release, explicit decision [FIXED: CON-WEB-0022, DEC-0031]
...
### D8 — What the canary gate evaluates [FIXED: DEC-0070]
...
| Journeys | `pnpm e2e:ci` — the Playwright journey suite (`specs/verification/journeys`, DEC-0040) in canary mode | fail on any red test |
| Route smoke | every path of the TS-WEB-0004 D1 inventory answers its expected status (200 for pages, 404 for the landing-only rule, 200 for `sitemap.xml` / `robots.txt` / `llms.txt`) | fail on any mismatch |
...
### D9 — The spec checker in CI; warnings are a burn-down [FIXED: DEC-0070]
...
because W3 is empty of meaning until tests exist at all (DEC-0040) and a
...
### D10 — Secrets and variables [FIXED: SRC-0012; names PROPOSED]
...
(DEC-0025 governs runtime tokens; this row is about CI). A workflow that
...
### D11 — Runner and toolchain [FIXED: DEC-0070]

`runs-on: ubuntu-latest`, no custom container. SRC-0012 runs its jobs in
...
| ID | Level | Check |
| --- | --- | --- |
| TS-WEB-0015-A1 | integration | With `VERCEL_ENV != "production"` or a non-canonical host: every response carries `X-Robots-Tag: noindex, nofollow`, `/robots.txt` disallows all, and the page emits the noindex meta tag. With production + canonical host: none of the three. |
| TS-WEB-0015-A2 | tool | The migration preview host answers unauthenticated requests with Vercel's protection challenge, and 200 only with a valid `x-vercel-protection-bypass` header. |
| TS-WEB-0015-A3 | static | `preview.yml` and `deploy.yml` declare the D4 / D7 jobs with exactly the specified `needs` edges, and neither carries a `paths-ignore` for `**/*.md`. |
| TS-WEB-0015-A4 | tool | A branch introducing a type error, a lint error, a failing test, an unused export, or a duplicated block above threshold turns the corresponding job red and leaves the PR unmergeable. |
| TS-WEB-0015-A5 | tool | `pnpm check` failing on an E-class error turns `Check` red; a run with only W1/W2/W3 warnings exits 0, and the warning block appears in the run summary and the sticky PR comment. |
| TS-WEB-0015-A6 | e2e | The Playwright journey suite runs green against the preview deployment URL reached through the bypass secret. |
| TS-WEB-0015-A7 | tool | On a green push to `main`: a canary production deployment is created, the D8 signals are evaluated against the canary, and `rolling-release/complete` is called exactly once. |
| TS-WEB-0015-A8 | tool | With a deliberately failing canary journey: no completion call is made, `rollback/{previousReadyProductionUid}` is called, the production alias serves the previous build, and an issue plus PR comment are created. |
| TS-WEB-0015-A9 | tool | Lighthouse CI, the bundle guard (TS-WEB-0003 D7) and the axe sweep (TS-WEB-0002 A1) run in `Budgets` after the preview deployment and before the merge gate resolves. |
| TS-WEB-0015-A10 | tool | A bot PR carrying the `auto-merge` label merges only after all five required checks are green; with one check red it stays open. |
| TS-WEB-0015-A11 | integration | `next.schafe-vorm-fenster.de` serves the head of `next-2026` after a push to that branch, protected and noindex. |
| TS-WEB-0015-A12 | manual | Per release: branch protection on `main` lists exactly the five D5 contexts, and every D10 secret and variable is present in the repository settings. |
...
- **When does `next.*` become free?** DEC-0035 calls the product's
...
- **Readiness probe.** SRC-0012 probes `/api/health`. The TS-WEB-0004 D5 BFF
  inventory has no health route, so D8 probes `/`. If a dedicated health
  route is preferred, TS-WEB-0004 D5 needs the amendment — a probe that
...
- **Burn-down governance.** D9 rejects a ratchet for now. Open: where is
  ...
  become a gate?
- **Content-triggered production deployments.** Q-0018 asks how a content
